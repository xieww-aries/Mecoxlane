;(function($){
	$.fn.zoom = function(options){
		...
		var _option = {
			...
			source_image_width: 560,  	// 放大图片的宽
			source_image_height: 730,	// 放大图片的高
		...
		if($ul.is("ul") && $ul.children("li").length && $ul.find(".etalage_big_image").length){
			...
			if(!$(".etalage_small_thumbs").length){
				...
				for(var i=0; i<_option.small_thumbs; i++){
					...
					if(i==0){
						html += '<li class="etalage_smallthumb_active"><img src="'+smurl+'" alt="small" style="width:'+100+'px; height:'+100+'px;" /></li>';
					}else{
						html += '<li style="opacity:0.4;"><img src="'+smurl+'" alt="small" style="width:'+100+'px; height:'+100+'px;" /></li>';
					}
				}
